...
- **ATP ranking alone achieves 65.6% accuracy on grass; the full model reaches 71.5% (+5.8pp lift, +8.7pp AUC).** ELO accounts for +3.0pp of that gain over raw ranking; the remaining +2.8pp comes from surface-specific serve stats, Grand Slam win rate, and isotonic probability calibration.
...
### Alcaraz withdrawal - draw updated
...
Trained on **all surfaces** (64,114 ATP / 58,694 WTA examples) with grass samples upweighted **3x**. Grass-specific features are masked to zero on hard/clay via an `is_grass` indicator so they only contribute on grass.

| Feature | Description |
|---------|-------------|
| `elo_all_diff` | Overall ELO difference |
| `elo_grass_diff` | Grass ELO diff x is_grass |
| `grass_wr_3y_diff` | 3-year grass win rate diff x is_grass |
| `form_365d_diff` | 1-year all-surface form diff |
| `form_90d_diff` | 90-day form diff |
| `grass_form_90d_diff` | 90-day grass form diff x is_grass |
| `bo5_wr_diff` | Grand Slam (best-of-5) win rate diff |
| `rank_diff` | Ranking difference |
| `age_diff` | Age difference |
| `ace_rate_diff` | Grass ace rate diff x is_grass |
| `first_serve_diff` | Grass 1st-serve % diff x is_grass |
| `bp_saved_diff` | Break points saved % diff x is_grass |
...
| AUC-ROC | 0.774 | 0.748 |
| Precision at 70%+ confidence | 79.6% | 76.3% |
